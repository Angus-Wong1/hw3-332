...
void myPrinterFunction(char *filePath, char *dirfile, FlagArgs flagArgs, int nestingCount)
{
    ...
    {
        if (strstr(dirfile, flagArgs.filterTerm) == NULL) // if the filter does notappear in the file
        {
            strcpy(line, ""); // clear the line print
        }
    }
...
void readFileHierarchy(char *dirname, int nestingCount, FileHandler 
*fileHandlerFunction, FlagArgs flagArgs)
{
